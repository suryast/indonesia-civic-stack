...
from civic_stack.djpb.normalizer import normalize_detail, normalize_search_row
...
def _extract_budget_data(soup: BeautifulSoup, keyword: str) -> list[dict]:
    ...
    if not rows:
        report_sections = soup.find_all(["div", "section"], {"class": lambda c: c and ("report" in c or "data" in c or "apbn" in c)})
        for section in report_sections:
            ...
            if keyword_lower in section_text:
                ...
                amount = section.find(["span", "p"], {"class": lambda c: c and ("amount" in c or "value" in c)})
                if amount:
