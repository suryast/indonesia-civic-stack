...
async def check_bpom(registration_no: str) -> dict:
    ...
    """
    from modules.bpom.scraper import fetch
...
async def search_bpom(keyword: str) -> list[dict]:
    ...
    """
    from modules.bpom.scraper import search
...
async def cross_reference_halal_bpom(product_name: str) -> dict:
    ...
    """
    from modules.bpjph.scraper import search as bpjph_search
...
async def search_companies_ahu(keyword: str) -> list[dict]:
    ...
    from modules.ahu.scraper import search
    results = await search(keyword)
    return [r.model_dump(mode="json") for r in results]
...
async def check_ojk_license(institution_name: str) -> dict:
    ...
    """
    from modules.ojk.scraper import fetch
...
async def search_ojk_institutions(keyword: str) -> list[dict]:
    ...
    from modules.ojk.scraper import search
    results = await search(keyword)
    return [r.model_dump(mode="json") for r in results]
...
async def search_oss_businesses(keyword: str) -> list[dict]:
    ...
    from modules.oss_nib.scraper import search
    results = await search(keyword)
    return [r.model_dump(mode="json") for r in results]
...
async def lookup_vendor_lpse(vendor_name: str) -> dict:
    ...
    from modules.lpse.scraper import fetch
...
async def search_lpse_vendors(keyword: str) -> list[dict]:
    ...
    from modules.lpse.scraper import search
    results = await search(keyword)
    return [r.model_dump(mode="json") for r in results]
...
async def get_candidate(candidate_id: str) -> dict:
    ...
    from modules.kpu.scraper import fetch
...
async def search_kpu_candidates(keyword: str) -> list[dict]:
    ...
    from modules.kpu.scraper import search
    results = await search(keyword)
    return [r.model_dump(mode="json") for r in results]
...
async def get_lhkpn(official_name: str) -> dict:
    ...
    """
    from modules.lhkpn.scraper import fetch
...
async def search_lhkpn(keyword: str) -> list[dict]:
    ...
    from modules.lhkpn.scraper import search
    results = await search(keyword)
    return [r.model_dump(mode="json") for r in results]
...
async def search_bps_datasets(keyword: str) -> list[dict]:
    ...
    """
    from modules.bps.scraper import search
    results = await search(keyword)
    return [r.model_dump(mode="json") for r in results]
...
async def get_bps_indicator(indicator_id: str, region_code: str = "0000") -> dict:
    ...
    """
    from modules.bps.scraper import get_indicator
...
async def list_bps_regions() -> dict:
    ...
    from modules.bps.scraper import list_regions
...
async def get_bmkg_alerts() -> dict:
    ...
    from modules.bmkg.scraper import get_alerts
    r = await get_alerts()
    return r.model_dump(mode="json")
...
async def get_weather_forecast(province: str = "DKIJakarta") -> dict:
    ...
    """
    from modules.bmkg.scraper import get_forecast
...
async def get_earthquake_history() -> list[dict]:
    ...
    from modules.bmkg.scraper import search
...
async def get_latest_earthquake() -> dict:
    ...
    from modules.bmkg.scraper import get_latest_earthquake as _get
    r = await _get()
...
async def lookup_building_permit(permit_id: str) -> dict:
    ...
    from modules.simbg.scraper import fetch
...
async def search_permits_by_area(keyword: str) -> list[dict]:
    ...
    from modules.simbg.scraper import search
    results = await search(keyword)
    return [r.model_dump(mode="json") for r in results]
