...
async def _download_pdf(client: httpx.AsyncClient, report_id: str) -> bytes | None:
    try:
        ...
        resp = await client.get(
            _PDF_URL, params={"id_laporan": report_id}, timeout=30.0
        )
        resp.raise_for_status()
...
def _extract_pdf_claude_vision(pdf_bytes: bytes) -> dict[str, Any]:
    ...
    if not api_key:
        raise EnvironmentError("ANTHROPIC_API_KEY is not set — required for Vision PDF fallback")
...
async def fetch(query: str, *, proxy_url: str | None = None) -> CivicStackResponse:
    ...
    if report_id:
        async with civic_client(proxy_url=proxy_url) as client:
            raw_detail = await _post_json(
                client, _DETAIL_URL, {"id_laporan": report_id}
            )
        if raw_detail:
...
async def compare_lhkpn(
    ...
    *,
    ...
    """
    async with civic_client(proxy_url=proxy_url) as client:
        data_a = await _post_json(
            client, _DETAIL_URL, {"id_laporan": f"{official_id}_{year_a}"}
        )
        data_b = await _post_json(
            client, _DETAIL_URL, {"id_laporan": f"{official_id}_{year_b}"}
        )
