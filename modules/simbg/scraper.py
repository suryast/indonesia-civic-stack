...
async def _search_national(client: httpx.AsyncClient, query: str) -> list[dict[str, Any]]:
    url = _SIMBG_NATIONAL + "/pbg/search"
    try:
        await _limiter.acquire()
        resp = await client.get(url, params={"keyword": query, "limit": 20}, timeout=15.0)
        resp.raise_for_status()
        data = resp.json()
        return data.get("data") or []
...
async def fetch(query: str, *, proxy_url: str | None = None) -> CivicStackResponse:
    ...
    best = deduped[0]
    confidence = 1.0 if not portal_failures else round(portal_successes / len(PILOT_PORTALS), 2)
...
async def search(keyword: str, *, proxy_url: str | None = None) -> list[CivicStackResponse]:
    ...
    portal_failures = [PILOT_PORTALS[i]["name"] for i, r in enumerate(portal_results_raw) if not r]
    confidence = 1.0 if not portal_failures else round(portal_successes / len(PILOT_PORTALS), 2)
